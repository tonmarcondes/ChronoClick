...
function assertProject(projectPath) {
  ...
  if (!fs.existsSync(path.join(resolved, "project.json")))
    throw new Error("Projeto ChronoClick inválido.");
  return resolved;
}
...
async function writeImage(dataUrl, file, options) {
  ...
  pipeline = pipeline.resize({
    ...
    height,
    fit: "inside",
    withoutEnlargement: options.upscale !== true,
  });
  if (options.format === "png") pipeline = pipeline.png({ compressionLevel: 8 });
  else if (options.format === "webp")
    pipeline = pipeline.webp({ quality: Number(options.quality || 82) });
...
async function createProject(message) {
  ...
  while (fs.existsSync(projectPath)) {
    slug = `${base}-${suffix++}`;
    projectPath = path.join(projectRoot, slug);
  }
  for (const folder of ["screenshots", "components", "documents"])
    fs.mkdirSync(path.join(projectPath, folder), { recursive: true });
  const config = message.config || {};
  const project = {
    schemaVersion: 2,
    id: crypto.randomUUID(),
    name: message.name || slug,
    slug,
  ...
  if (fs.existsSync(defaultTheme))
    fs.copyFileSync(defaultTheme, path.join(projectPath, "theme.css"));
...
async function saveEvent(message) {
  ...
  await writeImage(
    message.screenDataUrl,
    path.join(projectPath, "screenshots", screenName),
    screenOptions,
  );
  if (!group) {
    group = {
      id: step.groupId,
      page: step.page,
      screenshot: `screenshots/${screenName}`,
  ...
  if (message.microDataUrl) {
    ...
    await writeImage(
      message.microDataUrl,
      path.join(projectPath, "components", componentName),
      componentOptions,
    );
...
async function handle(message) {
  if (message.command === "ping")
    return { ok: true, version: require("../package.json").version, root: DEFAULT_ROOT };
  ...
  if (message.command === "eventChunk") {
    const upload = uploads.get(message.uploadId);
    if (!upload) throw new Error("Upload de evento expirou.");
    if (!["screen", "micro"].includes(message.kind)) throw new Error("Tipo de bloco inválido.");
    upload[message.kind].push(message.data);
    return { ok: true, received: message.index };
  }
  if (message.command === "commitEvent") {
    ...
    return {
      ...
      ...(await saveEvent({
        ...
        screenDataUrl: upload.screen.join(""),
        microDataUrl: upload.micro.join(""),
      })),
    };
  }
  if (message.command === "getSession") {
    ...
    return {
      ok: true,
      session: readJson(path.join(projectPath, "session.json")),
      project: readJson(path.join(projectPath, "project.json")),
    };
  }
  ...
  if (message.command === "saveSession") {
    ...
    project.config = message.session.config;
  ...
  if (message.command === "finish") {
    const projectPath = assertProject(message.projectPath);
    const file = path.join(projectPath, "session.json");
  ...
  if (message.command === "generateDocx") {
    ...
    if (recorded.captureFailures?.length && message.allowPartial !== true)
      throw new Error(
        "Há capturas com falha. Confirme a exportação dos passos salvos para continuar.",
      );
    if (!recorded.steps?.length)
      throw new Error(
        "Nenhum passo foi capturado. Inicie uma gravação e confira o contador de eventos antes de finalizar.",
      );
    if (
      !recorded.groups?.some((group) =>
        group.stepIds?.some((id) => recorded.steps.some((step) => step.id === id)),
      )
    )
      throw new Error(
        "A sessão não contém telas associadas aos passos. Não é possível gerar um documento vazio.",
      );
    for (const step of recorded.steps) {
      if (!step.images?.screen || !fs.existsSync(path.join(projectPath, step.images.screen)))
        throw new Error(`Print ausente no passo ${step.sequence}.`);
      if (step.images.microprint && !fs.existsSync(path.join(projectPath, step.images.microprint)))
        throw new Error(`Microprint ausente no passo ${step.sequence}.`);
    }
    const project = readJson(path.join(projectPath, "project.json"));
    const fileName =
      String(project.name || project.slug || "projeto")
        .replace(/[<>:"/\\|?*\u0000-\u001f]/g, "-")
        .replace(/[. ]+$/g, "")
        .trim() || "projeto";
    ...
    const args = [
      path.join(APP_ROOT, "cli", "generate-docx.cjs"),
      path.join(projectPath, "session.json"),
      output,
    ];
    if (fs.existsSync(path.join(projectPath, "theme.css")))
      args.push("--theme", path.join(projectPath, "theme.css"));
    const result = spawnSync(node, args, { env: process.env, encoding: "utf8", timeout: 120000 });
    if (result.error || result.status !== 0)
      throw new Error(result.error?.message || result.stderr || "Falha ao gerar DOCX.");
    if (!fs.existsSync(output) || fs.statSync(output).size < 100)
      throw new Error("A geração terminou sem um DOCX válido.");
  ...
  if (message.command === "openDocx") {
    ...
    if (
      !output ||
      path.dirname(path.resolve(output)) !== path.join(projectPath, "documents") ||
      path.extname(output) !== ".docx" ||
      !fs.existsSync(output)
    )
      throw new Error("Gere o DOCX antes de abrir o documento.");
...
process.stdin.on("data", (chunk) => {
  ...
  while (input.length >= 4) {
    ...
    Promise.resolve()
      .then(() => handle(message))
      .then((result) => respond({ requestId: message.requestId, ...result }))
      .catch((error) => respond({ requestId: message.requestId, ok: false, error: error.message }));
  }
});
