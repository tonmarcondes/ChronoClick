globalThis.ChronoPopup = {
  model(data, newProject = false) {
    ...
    if (generating || data.state === "finalizing")
      return {
        action: null,
        label: generating ? "Gerando DOCX…" : "Finalizando…",
        disabled: true,
        recording,
        canStart: false,
      };
    if (recording)
      return { action: "STOP", label: "Finalizar", disabled: false, recording, canStart: false };
    if (canStart)
      return {
        action: "START",
        label: "Iniciar nova",
